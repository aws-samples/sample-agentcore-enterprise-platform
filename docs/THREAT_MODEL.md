...
| UA-05 | Oversized, repeated, adversarial, or automated requests exhaust model tokens, concurrency, dependencies, or budget. | Denial of service and denial-of-wallet | Service quotas and telemetry are available. The local EBA Console bounds body/prompt size, concurrency, request rate, and client timeout. | Add production per-principal quotas, bounded context/steps, circuit breakers, budgets, anomaly alarms, and a tested emergency stop; local facilitator limits are not a hosted-service control. | `TBD / TBD` |
| UA-06 | Error messages, model output, or the local dashboard disclose tokens, prompts, topology, or sensitive diagnostics. | Confidentiality loss and further attack enablement | Dashboard collection uses a safe-field approach; the Playground returns bounded display text, metadata-only tool traces, and stable external errors. | Verify browser state, DOM, status files, logs, traces, errors, and CI artifacts with seeded secret/sensitive canaries; production still requires an approved redacted structured-logging policy. | `TBD / TBD` |
...
| EBA experience | Guided profiles, participant documentation, verification scripts, and a loopback-only facilitator console exist. The console distinguishes deployment mode, warns about synthetic data, pins the monitored account/Region/runtime, uses exact-origin CSRF protection, limits invocations, exposes only the customer-facing orchestrator, and has no create/delete surface. | Complete independent facilitation and accessibility tests; define participant isolation, approved evidence handling, fallbacks, handover, and verified cleanup. A local console is not a hosted multi-user control plane. |
