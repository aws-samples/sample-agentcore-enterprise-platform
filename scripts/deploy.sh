...
MODULE_EXPLAIN[4]=$'Identity Integration\nWhat: the AgentCore M2M credential provider agents use to reach the Gateway. Optional on top: your enterprise IdP federated into Cognito (set IDP_TYPE=entra_id|okta|ping) and 3LO providers for Google/GitHub/Notion (only when you supply their client ids).\nWhy: users keep their corporate logins, and agents get scoped credentials to act on their behalf with external services.'
