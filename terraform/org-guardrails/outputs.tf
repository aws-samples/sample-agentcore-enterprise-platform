...
output "attached_target_ids" {
  ...
  value       = (var.enable_scp_memory_enforce_cmk || var.enable_gateway_scps) ? var.target_ids : []
}

output "attachments_per_target" {
  description = "Number of SCPs this module attaches to each target (memory + consolidated gateway). Must stay <= 4 usable slots (5-per-target quota minus FullAWSAccess)."
  value       = (var.enable_scp_memory_enforce_cmk ? 1 : 0) + (var.enable_gateway_scps ? 1 : 0)
}
